...
public class CreditApplicationService {
     ...
     *  7. 限界上下文（BoundedContext）：限界就是领域的边界（定义了模型的适用范围），而上下文则是语义环境（确保语义的唯一性），用来封装通用语言和领域对象。
     ...
     *  16. 聚合根（AggregateRoot）：如果把聚合比作组织，那聚合根就是这个组织的负责人。聚合根也称为根实体，它不仅是实体，还是聚合的管理者。
     *  17. 聚合根还是聚合对外的接口人，以聚合根ID关联的方式接受外部任务和请求，在上下文内实现聚合之间的业务协同。
     *  18. 聚合就是由业务和逻辑紧密关联的实体和值对象组合而成的，聚合是数据修改和持久化的基本单元，每一个聚合对应一个仓储，实现数据的持久化。
     *  19. 聚合有一个聚合根和上下文边界，这个边界根据业务单一职责和高内聚原则，定义了聚合内部应该包含哪些实体和值对象，而聚合之间的边界是松耦合的。
     *  20. 聚合内有一套不变的业务规则，各实体和值对象按照统一的业务规则运行，封装真正的不变性实现对象数据的一致性。
     *  21. 聚合在DDD分层架构里属于领域层，领域层包含了多个聚合，共同实现核心业务逻辑。跨多个实体的业务逻辑通过领域服务来实现，跨多个聚合的业务逻辑通过应用服务来实现（包括微服务中）。
